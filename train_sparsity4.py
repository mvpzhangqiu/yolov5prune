...
def train(hyp, opt, device, tb_writer=None):
    ...
    if opt.adam:
        ...
        optimizer = optim.AdamW(pg0, lr=hyp['lr0'])  # adjust beta1 to momentum
    else:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, layer in model.named_modules():
            if isinstance(layer, nn.Conv2d):
                if i not in ignore_conv_list and not i.startswith("model.24"):
                    convw = layer.state_dict()['weight']
                    module_conv_list.append(convw)
            if isinstance(layer, nn.BatchNorm2d) and i not in ignore_bn_list:
                bnw = layer.state_dict()['weight']
                bnb = layer.state_dict()['bias']
                module_bn_list.append(bnw)
                module_bias_list.append(bnb)

        size_conv_list = [idx.data.shape[0] for idx in module_conv_list]
        size_bn_list = [idx.data.shape[0] for idx in module_bn_list]

        conv_weights = torch.zeros(sum(size_conv_list))
        bn_weights = torch.zeros(sum(size_bn_list))
        bnb_weights = torch.zeros(sum(size_bn_list))
        index = 0

        for idx, size in enumerate(size_conv_list):
            conv_weights[index:(index + size)] = module_conv_list[idx].data.abs().sum(dim=1).sum(dim=1).sum(dim=1).clone()
            index += size
        index = 0
        for idx, size in enumerate(size_bn_list):
            bn_weights[index:(index + size)] = module_bn_list[idx].data.abs().clone()
            bnb_weights[index:(index + size)] = module_bias_list[idx].data.abs().clone()
            index += size

        tb_writer.add_histogram('conv_weights/hist', conv_weights.numpy(), epoch, bins='doane')
        ...
        if rank in [-1, 0]:
            ...
            for x, tag in zip(list(mloss[:-1]) + list(results) + lr, tags):
                if tb_writer:
                    ...
                    tb_writer.add_scalar("sparity rate for conv weights", srctmp, epoch)  # tensorboard
                    tb_writer.add_scalar("sparity rate for bn gamma", srbtmp, epoch)  # tensorboard
                if wandb_logger.wandb:
