...
class ModelPrunedNoRes(nn.Module):
    def __init__(self, maskbndict, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        super(ModelPrunedNoRes, self).__init__()
        self.maskbndict = maskbndict
        if isinstance(cfg, dict):
            self.yaml = cfg  # model dict
        else:  # is *.yaml
        ...
        if anchors:
            logger.info(f'Overriding model.yaml anchors with anchors={anchors}')
            self.yaml['anchors'] = round(anchors)  # override yaml value
        self.model, self.save, self.from_to_map = parse_pruned_model_nores(self.maskbndict, deepcopy(self.yaml), ch=[ch])  # model, savelist
...
def parse_pruned_model_nores(maskbndict, d, ch):  # model_dict, input_channels(3)
    logger.info('\n%3s%18s%3s%10s  %-40s%-30s' % ('', 'from', 'n', 'params', 'module', 'arguments'))
    anchors, nc, gd, gw = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple']
    # gd, gw = 0.33, 0.50
    na = (len(anchors[0]) // 2) if isinstance(anchors, list) else anchors  # number of anchors
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv]:
            ...
            cv2out = int(maskbndict[named_m_cv2_bn].sum())
            cv3out = int(maskbndict[named_m_cv3_bn].sum())
            args = [cv1in, cv1out, cv2out, cv3out]
            chin = [cv1out]

            c2 = cv3out
            n = 1
            from_to_map[named_m_cv3_bn] = [named_m_cv1_bn, named_m_cv2_bn]
