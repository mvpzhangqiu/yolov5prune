...
def train(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
                    ...
                    if isinstance(m, nn.Conv2d) and not k.startswith("model.24"):
                        ...
                        if k not in ignore_conv_list:
                            for k in range(m.weight.data.shape[0]):
                                m.weight.grad.data[k].add_(srtmp * torch.sign(m.weight.data[k]))  # L1
            # print(mcv2_bn_list)
            # # ============================= sparsity training ========================== #
            # Optimize
           # if ni % accumulate == 0:
        ...
        for idx, size in enumerate(size_list):
            conv_weights[index:(index + size)] = module_list[idx].data.abs().sum(dim=1).sum(dim=1).sum(dim=1).clone()
            index += size
        ...
        if rank in [-1, 0]:
            ...
            for x, tag in zip(list(mloss[:-1]) + list(results) + lr, tags):
                if tb_writer:
                    ...
                    tb_writer.add_scalar("sr", srtmp, epoch)  # tensorboard
                if wandb_logger.wandb:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sr', type=float, default=0.00001, help='L1 normal sparse rate')
